...
//        $now = '2017-04-24';
        ...
        $query  = $analyticsDB->query("SELECT event_bidan_kunjungan_anc.baseEntityId,event_bidan_kunjungan_anc.locationId,event_bidan_kunjungan_anc.ancDate,event_bidan_kunjungan_anc.usiaKlinis,event_bidan_tambah_anc.tanggalHPHT,client_ibu.dusun,client_ibu.namaLengkap,event_bidan_identitas_ibu.NomorTelponHp FROM event_bidan_kunjungan_anc LEFT JOIN event_bidan_tambah_anc ON event_bidan_kunjungan_anc.baseEntityId=event_bidan_tambah_anc.baseEntityId LEFT JOIN client_ibu ON event_bidan_kunjungan_anc.baseEntityId=client_ibu.baseEntityId LEFT JOIN event_bidan_identitas_ibu ON event_bidan_kunjungan_anc.baseEntityId=event_bidan_identitas_ibu.baseEntityId WHERE event_bidan_kunjungan_anc.baseEntityId IN (SELECT baseEntityId FROM `event_bidan_tambah_anc` WHERE tanggalHPHT > '$batas') ORDER BY event_bidan_kunjungan_anc.ancDate ASC");
        ...
        foreach ($query->result() as $ibuhamil){
            ...
            if(array_key_exists($ibuhamil->baseEntityId, $result)){
                ...
                if($new_anc > $old_anc){
                    ...
                    $result[$ibuhamil->baseEntityId]["tlp"] = $ibuhamil->NomorTelponHp;
                ...
                if(array_key_exists($res['dusun'], $final_result[$res['locationid']])){
                    if($res["anc_ke"]==1){
                        if($res["ga"]>12&&$res["ga"]<=18){
                            array_push($final_result[$res['locationid']][$res['dusun']]["due"]["anc2"],$res);
                        }
                    }elseif($res["anc_ke"]==2){
                        if($res["ga"]>28&&$res["ga"]<=32){
                            array_push($final_result[$res['locationid']][$res['dusun']]["due"]["anc3"],$res);
                        }
                    }elseif($res["anc_ke"]==3){
                        if($res["ga"]>32&&$res["ga"]<=36){
                            array_push($final_result[$res['locationid']][$res['dusun']]["due"]["anc4"],$res);
                        }
                    }
                }
            }
...
//        $now = '2017-04-13';
        ...
        $query  = $analyticsDB->query("SELECT event_bidan_dokumentasi_persalinan.baseEntityId,event_bidan_dokumentasi_persalinan.locationId, client_ibu.namaLengkap, client_anak.birthDate, event_bidan_identitas_ibu.NomorTelponHp FROM event_bidan_dokumentasi_persalinan LEFT JOIN client_anak ON client_anak.ibuCaseId = event_bidan_dokumentasi_persalinan.baseEntityId LEFT JOIN client_ibu ON event_bidan_dokumentasi_persalinan.baseEntityId=client_ibu.baseEntityId LEFT JOIN event_bidan_identitas_ibu ON event_bidan_identitas_ibu.baseEntityId=event_bidan_dokumentasi_persalinan.baseEntityId  WHERE client_anak.birthDate > '$batas' AND keadaanBayi='hidup' AND event_bidan_dokumentasi_persalinan.locationId!='' GROUP BY event_bidan_dokumentasi_persalinan.baseEntityId");
        ...
        foreach ($query->result() as $ibunifas){
            ...
            $result[$ibunifas->baseEntityId]["tlp"] = $ibunifas->NomorTelponHp;
...
//                        var_dump($this->KN3_MESSAGE);
    ...
    private function intNumber($num){
      if(substr($num, 0,1)=='0'){
          $num[0] = '+';
          $num = str_replace('+',"+62",$num);
      }else{
